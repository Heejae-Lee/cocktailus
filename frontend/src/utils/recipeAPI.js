...
export const recipeAPI = {
  ...
  likeRequest: (
    ...
    axios
      .post(
        `/like`,
        { id: { article_id: id, member_name: member.name } },
        { headers: { "Auth-Token": `${member.token}` } }
      )
      .then(() => {
  ...
  likeRequestInDetail: (articleId, history) => {
    ...
    axios
      .post(
        `/like`,
        { id: { article_id: articleId, member_name: member.name } },
        { headers: { "Auth-Token": `${member.token}` } }
      )
      .then(() => {
        console.log("like success");
...
export const recipeCommentAPI = {
  deleteComment: (commentID, articleId, setComments, comments, closeModal) => {
    const member = JSON.parse(window.localStorage.getItem("memberData"));
    axios
      .delete(`/recipe-articles/${articleId}/comments/${commentID}`, {
        headers: { "Auth-Token": `${member.token}` },
      })
      .then(() => {
  ...
  writeComment: (articleId, comment, setNewComment, newComment) => {
    ...
    axios
      .post(
        `/recipe-articles/${articleId}/comments`,
        { article_id: articleId, content: comment, member_name: member.name },
        {
          headers: { "Auth-Token": `${member.token}` },
        }
      )
